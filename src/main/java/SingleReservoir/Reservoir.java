...
public class Reservoir {
    ...
    public NodeResult waterBalance() {
        ...
        Double evaporationLossWater;//扣除蒸散发水量
        Double ecologyWaterDemand;//生态需水
        Double ecologyWaterSupply;//生态供水
        //天然来水扣减蒸发
        evaporationLossWater = inputNatural * evaporationLossCoefficient;
        availableWater -= evaporationLossWater;
        //计算生态需水、供水
        ecologyWaterDemand = meanAnnualRunoff / 365.0 * timeStep / 24.0 * ecologicalCoefficient;
        ecologyWaterSupply = Math.min(ecologyWaterDemand, inputNatural - evaporationLossWater);
        availableWater -= ecologyWaterSupply;
        ...
        //double supply = 0.0;//每组供完水的水位
        double supplyAll = 0.0;
        double startSupplyStorage = 0.0;
        List<SupplyTarget> Targets = getSupplyTargetsByPriority();//拉入供水对象
        ...
        NodeResult WaterSupply = new NodeResult(effectiveSupplys,
                                                chargeDemand,
                                                storageFinal,
                                                evaporationLossWater,
                                                ecologyWaterSupply);
    ...
    public Map<String, Object> execute(Map<String, Object> inputParams,
        ...
        LocalDate currentDate = mapper.convertValue(inputParams.get("currentDate"), LocalDate.class);
        Reservoir reservoir = new Reservoir();
        try {
            // ========== 参数提取（按业务约定） ==========
            // 上游输入：本算子约定接收inputNatural TargetList
            Double storageIntial = mapper.convertValue(inputParams.get("storageIntial"), Double.class);
            Integer timeStep = mapper.convertValue(inputParams.get("timeStep"), Integer.class);
            Double inputNatural = mapper.convertValue(inputParams.get("inputNatural"), Double.class);
            List<SupplyTarget> targetList = mapper.convertValue(
                    inputParams.get("targetList"),
                    new TypeReference<List<SupplyTarget>>() {
                    });

            // enableDDT 控制流程：
            // true  -> 使用 DDT.applyCoefficientsNewList 先生成“新列表”（不改原对象）
            // false -> 直接使用原始 targetList
            boolean enableDDT = Boolean.TRUE.equals(mapper.convertValue(operatorParams.get("enableDDT"), Boolean.class));
            List<SupplyTarget> targetsForCalculation = targetList;
            if (enableDDT) {
                DDT ddt = mapper.convertValue(operatorParams.get("ddt"), DDT.class);

                if (ddt != null && currentDate != null && storageIntial != null && targetList != null) {
                    targetsForCalculation = ddt.applyCoefficientsNewList(currentDate, storageIntial, targetList);
                }
            }
            ...
            Double waterCharge = mapper.convertValue(inputParams.get("waterCharge"), Double.class);

            // 本算子配置
            String Id = mapper.convertValue(operatorParams.get("id"), String.class);
            Double siStorage = mapper.convertValue(operatorParams.get("siStorage"), Double.class);
            Double xunxianStorage = mapper.convertValue(operatorParams.get("xunxianStorage"), Double.class);
            Double zhengChangStorage = mapper.convertValue(operatorParams.get("zhengChangStorage"), Double.class);
            Double evaporationLossCoefficient = mapper.convertValue(operatorParams.get("evaporationLossCoefficient"), Double.class);
            Double meanAnnualRunoff = mapper.convertValue(operatorParams.get("meanAnnualRunoff"), Double.class);
            Double ecologicalCoefficient = mapper.convertValue(operatorParams.get("ecologicalCoefficient"), Double.class);
            Boolean isChargeBool = mapper.convertValue(operatorParams.get("isCharge"), Boolean.class);
            boolean isCharge = isChargeBool != null ? isChargeBool : false;
            DDX ddx = mapper.convertValue(operatorParams.get("ddx"), DDX.class);
            Double thisChargeStorage = 0.0;
            if (isCharge && ddx != null) {
            ...
            reservoir.setCurrentDate(currentDate);
